    ...
    <div v-if="design" class="flex flex-col md:flex-row flex-1">
      ...
      <div class="flex flex-col flex-1 max-w-[340px] pt-16 mr-12">
        ...
        >
          ...
          <p v-if="selectedCue && !selectedCue.published" class="text-xs text-amber-400 mt-2">
            ⚠ Unpublished
          </p>
        </div>
      </div>
          ...
          <div class="space-y-6">
            ...
            <div v-if="rpImageUrl" class="space-y-4">
              <div class="flex justify-between items-center">
                <label class="block text-sm text-ink-100">
                  Click to mark wrap boundaries ({{ rpLandmarks.length }}/2)
                </label>
                <button
                  v-if="rpLandmarks.length > 0"
                  @click="rpLandmarks = []"
              ...
              >
                ...
                <svg v-if="rpLandmarks.length === 2" class="absolute inset-0 w-full h-full pointer-events-none">
                  <line
                    :x1="`${rpLandmarks[0].xPercent}%`"
                    :y1="`${rpLandmarks[0].yPercent}%`"
                    :x2="`${rpLandmarks[1].xPercent}%`"
                    :y2="`${rpLandmarks[1].yPercent}%`"
                    stroke="white"
                    stroke-width="2"
            ...
            <div v-if="rpCanGenerate" class="space-y-4">
              ...
              <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <div
                  ...
                  @click="openCanvasPreview(rpListCanvas1)"
                >
                ...
                <div
                  class="bg-bg-100 rounded-lg p-2 cursor-pointer hover:bg-bg-200 transition-colors"
                  @click="openCanvasPreview(rpListCanvas2)"
                >
                ...
                <div
                  class="bg-bg-100 rounded-lg p-2 cursor-pointer hover:bg-bg-200 transition-colors"
                  @click="openCanvasPreview(rpDetailCanvas1)"
                >
                ...
                <div
                  class="bg-bg-100 rounded-lg p-2 cursor-pointer hover:bg-bg-200 transition-colors"
                  @click="openCanvasPreview(rpDetailCanvas2)"
                >
            ...
            <div class="flex gap-3">
              <button
                ...
                class="flex-1 py-3 border border-ink-200 rounded text-sm text-ink-100 hover:text-ink hover:border-ink transition-colors"
              >
                Cancel
              </button>
              <button
    ...
    <Transition name="modal">
      ...
      >
        <div class="bg-bg border border-ink-200 rounded-xl w-full max-w-md p-6">
          ...
          <p class="text-ink-100 text-sm mb-2">
            This will permanently delete the cue and all its images from storage.
          </p>
...
<script setup>
  ...
  const cueIdFromUrl = route.params.cueId || null;

  const { getDesignById, getCuesByDesignId, getPastCuesByDesignId, sortCues, getOptimizedImageUrl, refresh } = useCues();
  ...
  const totalImages = computed(() => (designCues.value.length > 0 ? 2 : 0));
  ...
  const getRawDetailImage = (cue, index) => {
    ...
    if (Array.isArray(details)) return details[index] || '';
    return details[index + 1] || '';
  };

  const getDetailImage = (cue, index) => {
    return getOptimizedImageUrl(getRawDetailImage(cue, index), { width: 1200, quality: 85 });
  };

  const getThumbnailImage = (cue, index) => {
    return getOptimizedImageUrl(getRawDetailImage(cue, index), { width: 200, quality: 80 });
  };
  ...
  const getRawPastCueImage = (pastCue) => {
    if (!pastCue) return '';
    if (pastCue.images?.main) return pastCue.images.main;
    if (pastCue.image) {
      if (pastCue.image.startsWith('http')) return pastCue.image;
  ...
  const getPastCueImage = (pastCue) => {
    return getOptimizedImageUrl(getRawPastCueImage(pastCue), { width: 800, quality: 85 });
  };

  const getPastCueThumbnail = (pastCue) => {
    return getOptimizedImageUrl(getRawPastCueImage(pastCue), { width: 200, quality: 80 });
  };
  ...
  const openModal = (imageSrc) => { modalImage.value = imageSrc; };
  const closeModal = () => { modalImage.value = null; };

  // ─── Navigation ────────────────────────────────────────────────────────────

  const onImageLoad = (imageIndex) => { loadedImages.value.add(imageIndex); };
  ...
  watch(selectedCueId, () => {
    if (selectedCueId.value !== null) loadedImages.value.clear();
  });
  ...
  const uploadBeautyImages = async () => {
    ...
    try {
      ...
      for (const img of pendingBeautyImages.value) {
        ...
        const path = `${basePath}/details/${nextIndex}.${ext}`;
  ...
  const saveReprocessed = async () => {
    ...
    try {
      ...
      // Keep existing beauty images (details[2+])
  ...
  const deleteCue = async () => {
    ...
    try {
      ...
      // Build list of storage paths to delete from the stored URLs
      const allUrls = [
        ...(cue.images?.list || []),
        ...(cue.images?.details || []),
      ];
      const storagePaths = allUrls
        .filter(Boolean)
        .map((url) => {
          const match = url.match(/\/cue-images\/(.+?)(\?|$)/);
      ...
      if (designCues.value.length > 0) {
        if (cueIdFromUrl) {
          ...
          if (cueExists) {
            ...
            const pastCueIndex = pastCues.value.findIndex((pastCue) => pastCue.id === cueIdFromUrl);
            if (pastCueIndex !== -1) {
        ...
        if (cueIdFromUrl) {
          const pastCueIndex = pastCues.value.findIndex((pastCue) => pastCue.id === cueIdFromUrl);
          selectedPastCueIndex.value = pastCueIndex !== -1 ? pastCueIndex : 0;
        } else {

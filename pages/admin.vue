  ...
  <div class="min-h-screen bg-bg text-ink p-8">
    ...
    <div v-else class="max-w-6xl mx-auto">
      ...
      <div class="flex justify-between items-center mb-8">
        ...
        <button @click="handleSignOut" class="text-sm text-ink-100 hover:text-ink underline">
          Sign Out
        </button>
      </div>
      ...
      <div class="space-y-6">
        ...
        <div>
          ...
          >
            ...
            />
            <p v-if="!sourceImage" class="text-ink-100">Click or drag to upload full cue image</p>
        ...
        <div v-if="imageUrl" class="space-y-4">
          ...
          >
            ...
            >
              <span
                class="absolute -top-5 left-1/2 -translate-x-1/2 text-xs text-white bg-red-500 px-1 rounded"
              >
                {{ index === 0 ? "Start" : "End" }}
              </span>
            </div>
...
<script setup>
  ...
  const generateListImage2 = (img, wrapStart, wrapEnd) => {
    ...
    const canvasWidth =
      visiblePixelWidth * Math.cos(absAngle) + img.naturalHeight * Math.sin(absAngle);
    const canvasHeight =
      visiblePixelWidth * Math.sin(absAngle) + img.naturalHeight * Math.cos(absAngle);
  ...
  const generateDetailImage2 = (img) => {
    ...
    const absAngle = Math.abs(angle);
    const rotatedWidth =
      img.naturalWidth * Math.cos(absAngle) + img.naturalHeight * Math.sin(absAngle);
    const rotatedHeight =
      img.naturalWidth * Math.sin(absAngle) + img.naturalHeight * Math.cos(absAngle);
  ...
  const generateAndSave = async () => {
    ...
    try {
      ...
      const { error: insertError } = await $supabase.from("cues").insert({
        id: cueIdNum,
